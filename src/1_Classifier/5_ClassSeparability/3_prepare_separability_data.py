"""
Prepare the frozen coarse-label mapping and cluster annotation subsets.
...
With the current frozen configuration, 35 original classes are merged into
eight coarse classes. All labels below are zero-based PyTorch labels:

1. cutworms (coarse label 0)
   - 18: black cutworm
   - 19: large cutworm
   - 20: yellow cutworm

2. plant_hoppers (coarse label 1)
   - 7: brown plant hopper
   - 8: white backed plant hopper
   - 9: small brown plant hopper

3. fruit_flies (coarse label 2)
   - 83: Tetradacus c Bactrocera minax
   - 84: Dacus dorsalis(Hendel)
   - 85: Bactrocera tsuneonis

4. army_worms (coarse label 3)
   - 23: army worm
...
5. blister_beetles (coarse label 4)
   - 49: lytta polita
   - 50: legume blister beetle
   - 51: blister beetle

6. aphids (coarse label 5)
   - 24: aphids
...
8. plant_bugs_and_miridae (coarse label 7)
   - 57: Apolygus lucorum
...
These lists document the current frozen configuration. The actual source of
truth is selected_clusters.csv, which must be created and frozen using
validation results before this script or any test-set evaluation is run.
"""
